...
ASSETS=$(docker exec -i ***REMOVED-SECRET*** psql -U admin -d ***REMOVED-SECRET*** -t -A -c "
...
for ASSET in $ASSETS; do
    ...
    # Obtener el asset_id
    ASSET_ID=$(docker exec -i ***REMOVED-SECRET*** psql -U admin -d ***REMOVED-SECRET*** -t -A -c "
        SELECT id FROM infra_inventory WHERE asset_name = '$ASSET';
    ")

    if [ ! -z "$ASSET_ID" ]; then
        # 1. Resolver todas las vulnerabilidades del activo
        docker exec -i ***REMOVED-SECRET*** psql -U admin -d ***REMOVED-SECRET*** -c "

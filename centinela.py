...
VALKEY_CONFIG = {
    "host": get_secret("VALKEY_HOST", "***REMOVED-SECRET***"),
    "port": 6379,
    "db": 0
}
...
def try_init_provider(p):
    ...
    try:
        ...
        if p in ("google_genai", "vertex_ai") and genai is not None:
            ...
            base_url = get_secret("OLLAMA_BASE_URL", "***REMOVED-SECRET***")
            llm = ChatOllama(base_url=base_url, model=model_name)
            return True
        elif p == "nvidia_nim" and ChatOpenAI is not None:
            api_key = get_secret("NVIDIA_NIM_API_KEY")
            base_url = get_secret("NVIDIA_NIM_BASE_URL", "***REMOVED-SECRET***")
            ...
            use_model = get_secret("AI_MODEL_NVIDIA", "***REMOVED-SECRET***")
            if api_key:

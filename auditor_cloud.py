...
CREDENTIALS_PATH = "***REMOVED-SECRET***"
...
def run_prowler(asset_id: int, provider: str = "gcp") -> list[dict]:
    ...
    # Base command: prowler gcp --credentials-file ***REMOVED-SECRET***
    ...
    if provider == "gcp":
        if os.path.exists(CREDENTIALS_PATH):
            ...
            print("ℹ️ [Prowler] ***REMOVED-SECRET*** not found, using default env/metadata credentials")
...
def run(asset_id: int, asset_name: str, provider: str = "gcp"):
    ...
    if not findings:
        # Log audit indicating no cloud issues or Prowler structure ready
        with db_manager.get_db_cursor() as cur:
            cur.execute("""
                INSERT INTO vulnerability_log (asset_id, cve_id, severity, description, status, scan_engine)
                VALUES (%s, 'PROWLER-AUDIT', 'Info', 'Estructura Prowler CSPM verificada. Sin configuraciones erróneas detectadas o credenciales no provistas.', 'NEW', 'prowler')
